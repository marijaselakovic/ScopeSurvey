...
{\bf mb:[Marija, I don't understand the point about the compiler not providing any instrumentation. What do we care? We want to do historical analysis on lots of jobs, so we need stats from previous jobs. Right?]}
...
After execution of a SCOPE job finishes, the runtime produces several artifacts that contain code and runtime information for every job stage. Job artifacts are indefinitely stored within Cosmos itself in a {\em job repository}. We provide an overview of the subset of artifacts which we use to profile the data center.
